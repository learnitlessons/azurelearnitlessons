...
# Function to create VMs with static IPs
function Create-VMWithStaticIP {
    ...
    # Changed VM size from Standard_B1s to Standard_B2s for 4GB RAM and 2 vCPUs
...
# Function to create a member server and optionally join it to the domain
function Create-MemberServer {
    ...
    $pip = New-AzPublicIpAddress -Name "$serverName-pip" -ResourceGroupName $resourceGroup -Location $location -AllocationMethod Static -Sku Standard
    $nic = New-AzNetworkInterface -Name "$serverName-nic" -ResourceGroupName $resourceGroup -Location $location -SubnetId $vnet.Subnets[0].Id -PublicIpAddressId $pip.Id -PrivateIpAddress $staticIP
    $nic.IpConfigurations[0].PrivateIpAllocationMethod = "Static"
    Set-AzNetworkInterface -NetworkInterface $nic

    $nsg = New-AzNetworkSecurityGroup -ResourceGroupName $resourceGroup -Location $location -Name "$serverName-nsg" -SecurityRules (New-AzNetworkSecurityRuleConfig -Name "RDP" -Protocol Tcp -Direction Inbound -Priority 1000 -SourceAddressPrefix * -SourcePortRange * -DestinationAddressPrefix * -DestinationPortRange 3389 -Access Allow)
    $nic.NetworkSecurityGroup = $nsg
    Set-AzNetworkInterface -NetworkInterface $nic

    # Changed VM size from Standard_B1s to Standard_B2s for 4GB RAM and 2 vCPUs
    $vmConfig = New-AzVMConfig -VMName $serverName -VMSize "Standard_B2s" | 
        Set-AzVMOperatingSystem -Windows -ComputerName $serverName -Credential (New-Object PSCredential($user, $securePass)) | 
        Set-AzVMSourceImage -PublisherName "MicrosoftWindowsServer" -Offer "WindowsServer" -Skus "2022-datacenter-azure-edition" -Version "latest" | 
        Add-AzVMNetworkInterface -Id $nic.Id | 
        Set-AzVMBootDiagnostic -Disable |
        Set-AzVMOSDisk -CreateOption FromImage -StorageAccountType StandardSSD_LRS
    ...
    Write-Host "Member server $serverName created successfully."
    Write-Host "Public IP: $($pip.IpAddress)"
    Write-Host "RDP command: mstsc /v:$($pip.IpAddress) /u:$user"

    if ($joinDomain) {
        $domainJoinScript = @"
        `$domainAdminPassword = ConvertTo-SecureString "$domainAdminPassword" -AsPlainText -Force
        `$domainAdminCred = New-Object System.Management.Automation.PSCredential ("$domainAdminUser", `$domainAdminPassword)
        Add-Computer -DomainName $domainName -Credential `$domainAdminCred -Restart -Force
"@
        $result = Run-AzVMCommand -resourceGroup $resourceGroup -vmName $serverName -script $domainJoinScript
        if ($result.Status -eq "Succeeded") {
            Write-Host "$serverName successfully joined to the domain $domainName"
        } else {
            Write-Host "Failed to join $serverName to the domain $domainName"
        }
    }
}
...
# New function to create a member server and optionally join it to the domain
function Create-MemberServer {
    param (
        [string]$location,
        [string]$resourceGroup,
    ...
    $vmConfig = New-AzVMConfig -VMName $serverName -VMSize "Standard_B1s" | 
        Set-AzVMOperatingSystem -Windows -ComputerName $serverName -Credential (New-Object PSCredential($user, $securePass)) | 
        Set-AzVMSourceImage -PublisherName "MicrosoftWindowsServer" -Offer "WindowsServer" -Skus "2022-datacenter-azure-edition" -Version "latest" | 
        Add-AzVMNetworkInterface -Id $nic.Id | 
        Set-AzVMBootDiagnostic -Disable |
...
do {
    ...
    Write-Host "6. Create Member Server"
    Write-Host "7. Exit"
    ...
    switch ($action) {
        ...
        "6" {
            do {
                Write-Host "`nSelect the region to create a member server:"
                for ($i = 0; $i -lt $regions.Count; $i++) {
                    Write-Host "$($i+1). $($regions[$i].name)"
                }
                ...
                switch ($choice) {
                    "1" { $selectedRegion = $regions[0] }
                    "2" { $selectedRegion = $regions[1] }
                    "3" { $selectedRegion = $regions[2] }
                    "4" { break }
                    default { Write-Host "Invalid choice. Please try again."; continue }
                }

                if ($choice -ne "4") {
                    $serverName = Read-Host "Enter the name for the member server"
                    $joinDomain = Read-Host "Do you want to join the server to the domain? (Y/N)"
                    $joinDomain = $joinDomain -eq "Y"

                    if ($joinDomain) {
                        $domainAdminUser = Read-Host "Enter the domain admin username"
                        $domainAdminPassword = Read-Host "Enter the domain admin password" -AsSecureString
                        $domainAdminPassword = [System.Runtime.InteropServices.Marshal]::PtrToStringAuto([System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($domainAdminPassword))
                    } else {
                        $domainAdminUser = ""
                        $domainAdminPassword = ""
                    }
                    ...
                    Create-MemberServer -location $selectedRegion.location `
                                        ...
                                        -addressPrefix $selectedRegion.addressPrefix `
                                        -serverName $serverName `
                                        -staticIP $selectedRegion.memberServerIP `
